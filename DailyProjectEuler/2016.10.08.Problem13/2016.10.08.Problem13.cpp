...
void AddNumberToResult(int result[], int column, int number)
{
	int currentResult = result[column];
	int newResult = currentResult + number;
	...
	if (newResult > 9)
	{
		...
		int leftDigits = newResult / 10;
		AddNumberToResult(result, column - 1, leftDigits);
	}
}
...
long long getAnswer()
{
	...
	for (int j = 49; j >= 0; j--)
	{
		...
		for (int i = 0; i < 100; i++)
		{
			...
			columnSum = columnSum + currentNumber;
		}

		AddNumberToResult(result, resultColumn, columnSum);
	...
	for (int j = 0; j < 10; j++)
	{
		...
		resultNumber = resultNumber + result[index];
	}
